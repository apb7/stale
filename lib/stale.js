...
module.exports = class Stale {
  ...
  async mark (type, issue) {
    ...
    const staleLabel = this.getConfigValue(type, 'staleLabel')
    var markComment = this.getConfigValue(type, 'markComment')
    const number = issue.number
    const pullRequestAuthor = issue.user.login
    var customMarkComment = markComment.replace('@author', '@' + pullRequestAuthor)
    if (perform) {
